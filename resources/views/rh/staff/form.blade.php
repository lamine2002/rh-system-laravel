...
@section('contents')
    ...
    <div class="mt-8">
        <form action="{{ $staff->exists ? route('rh.staff.update', $staff) : route('rh.staff.store') }}" method="POST">
            ...
            {{-- Ajouter champ chef_id et team_id            --}}
            <div class="grid grid-cols-2 gap-4 mt-4">
                <div>
                ...
                <div>
                    <label for="chief_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Chef</label>
                    <select name="chief_id" id="chief_id"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <option>Choisir le chef</option>
